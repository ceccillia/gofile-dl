...
class Downloader:
    ...
    def _get_total_size(self, link):
        r = requests.head(link, proxies=self.proxy, headers={"Cookie": f"accountToken={self.token}"})
        r.raise_for_status()
    ...
    def _download_range(self, link, start, end, temp_file, i):
        ...
        with requests.get(link, proxies=self.proxy, headers=headers, stream=True) as r:
            r.raise_for_status()
...
class GoFile(metaclass=GoFileMeta):
    ...
    def update_token(self) -> None:
        if self.token == "":
            data = requests.post("https://api.gofile.io/accounts", proxies=self.proxy).json()
            if data["status"] == "ok":
    ...
    def update_wt(self) -> None:
        if self.wt == "":
            alljs = requests.get("https://gofile.io/dist/js/global.js", proxies=self.proxy).text
    ...
    def execute(self, dir: str, content_id: str = None, url: str = None, password: str = None, num_threads: int = 1, excludes: list[str] = None, proxy: str = None) -> None:
        self.proxy = {"http": proxy, "https": proxy} if proxy else None 
        files = self.get_files(dir, content_id, url, password, excludes, proxy)
        for file in files:
            Downloader(token=self.token, proxy=proxy).download(file, num_threads=num_threads)

    def get_files(self, dir: str, content_id: str = None, url: str = None, password: str = None, excludes: list[str] = None, proxy: str = None) -> None:
        if excludes is None:
            excludes = []
        files = list()
...
if __name__ == "__main__":
    ...
    GoFile().execute(dir=dir, url=args.url, password=args.password, num_threads=num_threads, excludes=args.excludes, proxy=args.proxy)
